...
> Local Foundation 与上述柜台能力已进入 `main`，但**完整柜台工作日尚未经真实 PostgreSQL 端到端验收**：当前真实 PG 覆盖集中在身份、RLS、迁移与照片链路，开单/收款/取衣/交班的服务端测试仍走 capturing pool，端到端 E2E 只覆盖登录进入柜台壳。真实工作日验收是后续 Acceptance 切片的范围；模拟打印、云部署与 Windows 适配继续延后。
