...
- 推荐奖励与团购券核销（[ADR-54](adr/2026-08-13-adr-54-referral-and-group-buy.md)）：店主可基于被推荐人
  已结清且实付的订单，为推荐人的 active 会员账户发放既有优惠券；服务端锁后复核活动窗口、双方账户、
  订单、券版本和预算，并让 grant、推荐证据与等额预算流水原子落账。外部团购券只保存本机生成的域分离
  SHA-256 摘要和末四位，单券只能核销到一个未付款开放订单，优惠金额由服务端按面值与订单原价较小值
  计算。三条写入口均为 online-only R4、当前门店 marketing feature、专用限流和 WYSIWYS 漂移复核；
  卡片逐字段展示公开 authority，迟到请求在会话、活动或输入变化后失效。推荐的完整同语义重放在预算
  耗尽、活动结束或券退役后仍只返回首次证据，语义漂移失败关闭；原码不进入 HTTP/pending/数据库/审计。
  本片不包含推荐采集/风控、provider API/结算/退款、通知或顾客自助。
...
- 活动批量发券与核销冲正（[ADR-53](adr/2026-08-13-adr-53-campaign-coupon-issuance.md)）：店主从已冻结
  ...
  误核销只能在未付款 open 订单上由另一管理员 R4 复核后追加冲正，原核销证据不会删除或改写。
  复核卡冻结活动/snapshot、券版本/名称/最低消费/有效期和预算；输入在请求期间变化时，迟到响应不会
  打开旧确认卡，并发发券会在活动锁后按最新已用预算重新确认。
  Owner 页面提供 aggregate 发券预览和按核销 ID 冲正的受控入口，不接收或缓存顾客名单；本片不发送
  通知；推荐/团购现由 ADR-54 独立切片承接，仍不等于顾客自助券包已经交付。

- 门店营销活动基础（[ADR-52](adr/2026-08-13-adr-52-store-marketing-campaigns.md)）：店主可在 marketing
  feature 显式开启后，按严格白名单规则定义当前门店活动、1–500 人受众上限、最长 730 天时间窗和
  1–5,000,000 分整数预算，并预览/冻结不含顾客名单的受众摘要。活动、摘要和预算 authority 全部
  FORCE store RLS、乐观版本、持久幂等和同事务审计；高预算升级另一管理员复核。该基础本身不发券；
  发券/冲正由 ADR-53 的独立 R4 入口承接，通知发送仍未包含。
