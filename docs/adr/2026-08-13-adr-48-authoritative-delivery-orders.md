...
canonical 顾客和可选的取/送预约；当时不创建配送任务，也不保存照片、签名、GPS 或路线。后续
[ADR-49](2026-08-13-adr-49-authoritative-delivery-tasks.md) 已为 Item 4 增加独立任务保管链，但不改变
本 ADR 的订单真源；移动任务面仍属于 Item 5，交付证据仍属于 Item 6。
...
- **让任务状态成为配送订单真源**：即使后续 ADR-49 已增加任务和人工接管链，订单仍必须是路线进度
  真源；任务只证明当前执行人和分派历史，否决反转权威关系。
- **创建时接受客户端运费或初态**：可绕过策略快照与路线约束，否决；从预约/路线派生。
- **配送完成顺便修改所有衣物和订单**：绕开既有件级状态机、支付与审计命令，否决；只验证权威终态。
- **feature 关闭后禁止收尾**：会冻结已取走衣物，否决；只阻断新建。
- **现在保存路线、GPS、照片或签名**：仍分别属于 Item 5/6，否决并后置；Item 4 的员工任务保管链由
  ADR-49 单独交付。
