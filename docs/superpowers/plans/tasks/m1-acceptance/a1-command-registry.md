...
| `max_batch?` | `number`「批量上限（触发 R3→R4 升级阈值）」 | **上限与阈值语义冲突 + 缺金额阈值 + 缺规模求值方式，见 §3.2（P0）** |
...
同一行注释里"批量上限"（cap，超了拒绝）与"触发 R3→R4 升级阈值"（escalate，超了升级）是**两种不同行为**，实现方必须二选一。且 §6.5 与 ADR-05 第 4 条都要求"数量**/金额**阈值触发升级"（例："单笔调整 >100 元"），而元数据四字段里**没有金额阈值字段**。

→ **ADR-09 决策 2（含修订 1）：一拆为三**——`size_measures`（规模怎么从入参算）/ `hard_limits`（超即拒）/ `risk_escalation`（超即升 R4）。两条良构约束构造时校验：①任一维有阈值则该维 `size_measures` 必填；②`risk_escalation.max_* ≤ hard_limits.max_*`，否则 R4 通道是死代码。附带约束："阈值 per-org 可配但只能调严"（§6.5）必须在契约层可表达、可测。

> 修订 1 的 `size_measures` **采纳自 Codex 本组设计稿**——其指出批量规模因命令而异（数组长度／数量合计／金额与件数联合），ADR-09 初稿只给阈值不给求值方式，C5 只能逐命令硬编码。该缺口成立。
...
- [ ] §3.1/§3.2 按 ADR-09（含修订 1）落地：`offline_mode` 三值枚举；`size_measures` / `hard_limits` / `risk_escalation` 三字段并存。
- [ ] 两条良构约束**有实际断言**：缺 `size_measures` 而有阈值 → 构造失败；`risk_escalation.max_batch > hard_limits.max_batch` → 构造失败。
