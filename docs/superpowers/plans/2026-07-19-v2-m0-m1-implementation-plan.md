# laundry-v2 实施计划：V2-M0 技术验证 + V2-M1 基座（含四 AI 分工）

> 日期：2026-07-19　作者：Claude（设计与门禁）
> 依据：架构 spec draft3.1a（已定稿）、ADR-01…08（全部 Accepted）、UI spec draft3.1a
> 范围：本文件只细化**即将开工的两期**（M0 验证、M1 基座）到任务级，并给出四 AI 分工机制。M2–M6 的范围见 ADR-07，各期开工前再出同粒度计划。
> 角色边界：Claude 负责设计/门禁/评审，不写实现代码（CLAUDE.md）；实现由 Codex/Gemini/Grok 承担，Claude 每期验收、关键节点 Codex 二审。
...
#### 包 A · `packages/contracts`（契约先行，最先冻结）— **Claude 评审 + Codex 落地**
...
- **冻结门禁**：Claude 评审通过打 tag `contracts@v0.1.0`；此后改契约走 ADR。

#### 包 B · `packages/domain`（纯函数 + TDD）— **Gemini**
...
#### 包 C · `apps/server`（Fastify + 命令总线）— **Codex（基座）+ Gemini（identity/platform 服务）**
...
#### 包 D · `apps/edge-agent`（Edge v0）— **Grok**
...
#### 包 E · `apps/web`（登录 + 骨架）— **Grok（与 D 同属 Grok，或前端专人）**
...
#### 包 F · `tools/`（迁移器骨架 + 种子 + compose）— **Gemini**
...
### 2.2 M1 集成门禁（Claude 验收 + Codex 二审）
...
机制先于人选已定（架构 §15）；此处按各 AI 的定位给出**目录所有权**建议。原则：一目录一 AI、跨模块只经契约包、动 git 前探测活跃编辑、审计用独立 worktree（三 AI 共 checkout 的既有教训）。

### 3.1 角色定位

| AI | 定位 | 主责 | 理由 |
|---|---|---|---|
| **Claude** | 设计与门禁 | contracts 评审冻结、每期验收、Code Review、ADR、产品文档 | CLAUDE.md 既定角色，不写实现代码 |
| **Codex** | 安全与基座 + 二审 | 命令总线运行时 · RLS/Postgres/迁移 · Edge 密码学（lease/DEK/票据）· 关键节点二审 | 已连任五轮安全评审，安全直觉与严谨性最契合基座与密码学 |
...
- **契约冻结日**（每期开工首日）：Claude 评审 `packages/contracts` 打 tag；实现方以该 tag 为唯一契约来源，期内改契约走 ADR。
- **目录锁**：动某目录前在团队频道声明；跨模块需求提 issue 走契约，不直接改他人目录。
- **集成日**（每周五）：全量 E2E + 契约测试；红灯当日修。
- **PR 规则**：每模块 PR 必带契约测试 + 种子数据 + 门禁自查清单；Claude 验收，命中关键节点加 Codex 二审。
- **分支**：trunk-based 短分支；禁止直接推 main。

### 3.3 依赖顺序（谁等谁）

```text
Claude 冻结 contracts@v0.1.0
   ├─→ Gemini: domain(B) 可立即并行（纯函数，只依赖 contracts 类型）
   ├─→ Codex: server 命令总线(C1–C5) 依赖 contracts + domain 骨架
   │       └─→ Gemini: identity/platform 服务(C6–C7) 依赖总线就绪
   ├─→ Grok: edge-agent(D) 依赖 contracts 的桥协议(A4)，可与 server 并行
   └─→ Grok: web(E) 依赖 contracts 的 OpenAPI 类型(A7)，可与 server 并行
   Gemini: tools/compose(F) 尽早，给所有人本地环境
```

关键路径 = contracts 冻结 → 命令总线 → identity 服务 → web 登录闭环；Edge 与 domain 并行不阻塞。

---

## 4. 里程碑之后（M2–M6 提要，详见 ADR-07）

| 期 | 一句话范围 | 首要 AI |
|---|---|---|
| M2 | 柜台核心（件级开单/取衣/三类打印/离线闭环）+ 只读 AI + BYOK 最小闭环 | Gemini(服务) + Grok(web/edge) |
| M3 | 会员储值 + 通知/催取 + 开放 R3 写 | Gemini + Codex(通知安全) |
| M4 | 账务双口径 + 交班 + 老板端 + 备份还原 + 分析类 AI | Gemini + Grok |
| M5 | BYOK 全矩阵 + 异步审批中心 + 有边界自动化 | Codex(安全) + Gemini |
| M6.1–6.4 | 视觉 AI / 小程序 / 工厂协同 / 取送营销（四子期独立门禁） | 按端拆分 |

每期开工前，Claude 出本期同粒度计划 + 冻结当期 contracts 增量。

---

## 5. 立即可执行的下一步

1. **本 PR**：v2 设计（specs/ADR）+ 本实施计划落入仓库，合入 main 作为 v2 启动基线。
2. **人选拍板**：manpengan 确认 §3.1 四 AI 归属（或调整）。
3. **M0 开工**：确定后按 §1.1 矩阵分派六个 spike；宏发现役三台打印机是 M0-3 的现成实机。
4. **contracts 冻结**：M0 findings 回来、无阻塞设计变更后，Claude 评审冻结 `contracts@v0.1.0`，M1 正式开工。
