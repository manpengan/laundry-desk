...
`AGENTS.md` 将单文件 200–400 行定义为推荐范围、800 行定义为硬上限。Local
Foundation 在保持测试与安全边界的同时产生了 12 个新跨过 400 行的生产文件，
但当前 ESLint 没有自动执行文件规模门禁，因此新文件和既有大文件都可能继续增长。
...
门禁进入默认 `workspace:lint` 和 `workspace:test`，不能只依赖人工复审。
...
其中 `packages/contracts/src/index.ts` 在 `main` 已超过 400 行，其余 12 个是当前
分支新跨过推荐范围的文件。预算是技术债上限，不是长期豁免。
...
- 根 `.eslintrc.cjs` 对 `tools/local` 与 `tests/foundation` 使用同样规则，并为
  `tools/local/config.mjs` 声明冻结预算；
...
4. 运行 `pnpm workspace:check`，确认格式、lint、strict typecheck、测试与构建全部
   通过。

## 5. 非目标

- 本轮不把 13 个既有大文件全部重构到 400 行以内；
