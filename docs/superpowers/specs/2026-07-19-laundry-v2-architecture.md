...
>
> 本文件是设计，不含实现代码。自 ADR-10 起由 Codex 单一负责设计与核心实现，Grok 按 §15 在冻结接口下协助端侧/平台/硬件。
...
## 15. 单一技术负责人交付机制（ADR-10）

1. **Codex 单一负责**：后续设计真源、contracts、domain、server、PG/migrations、安全核心、集成和门禁由 Codex 统一负责。
2. **Grok 受约束协助**：只在冻结 contracts/ports 下实现 Web/UI、Edge platform/drivers/packaging、硬件与 Windows 实测；不得自行改变密码学、租户、lease、审计或 Policy 语义。
3. **ports/adapters 隔离**：Edge core/security/queue/lease 由 Codex 所有，platform/drivers/packaging 由 Grok 所有；Web 只消费 A7 生成类型。
4. **历史资产不抹除**：已合入成果继续有效；Claude/Gemini 退出关键路径，未合分支只作候选输入，可选复审不阻塞。
5. **门禁**：TS strict 零错、ESLint 零警、文件 ≤400 行、函数 ≤50 行、嵌套 ≤4、金额零浮点、覆盖 ≥70%、契约测试、跨租户负向、身份负向、AI 红队、WYSIWYS 与 Playwright 全绿；外部实机证据单独记录。
6. 每项独立 worktree/短分支/PR；Codex 提交合并建议，manpengan 最终合并，Codex 再验证 main 同提交 CI。

详见 [交付治理](2026-07-21-laundry-v2-delivery-governance.md) 与 [ADR-10](../../adr/2026-07-21-adr-10-single-owner-delivery-governance.md)。
