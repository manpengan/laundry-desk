...
> 日期：2026-08-11
> 当前状态：**实现与本地完整门禁已通过；PR/required CI/hk-vps 发布 pending**
...
- 本记录把 Contracts/Domain、Server、PostgreSQL、Browser、GitHub CI 与 hk-vps 发布分层；
  任一层绿灯都不代替其他层。
- 当前只记录未提交候选上的本地新鲜证据。只有变更合入 `main`、required CI 全绿并把该精确
  merge SHA 完成 `prepare → finalize` 后，阶段 2 才能标记关闭。
...
| GitHub PR           | **pending** | 精确 PR head 的 required `workspace-check`、`real-postgres`、`runtime-app-macos` 全绿并合入                |
| 合并后主干          | **pending** | 精确 merge SHA 的 push 级 Foundation 与 PostgreSQL Integration 成功                                        |
| hk-vps 两阶段发布   | **pending** | 精确 merge SHA、migration head 0047、恢复点、shadow/catalog、API 与 Cloud Chromium evidence 全部 committed |
| 发布后审计          | **pending** | marker/runtime/schema 一致；Desk/PG/Caddy/KB 健康；loopback 边界、审计与合成数据清理通过                   |
