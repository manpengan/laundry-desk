...
本记录形成时不包含配送任务分派、接单/转派、人工接管、移动 H5、路线、GPS、照片、签名、通知、
顾客本人认证/公开自助、离线/Edge 或第三方 provider。后续 Item 4 已由
[ADR-49](../../adr/2026-08-13-adr-49-authoritative-delivery-tasks.md)及其独立验收记录增加任务保管链；
该后续能力不会反向把任务变成配送订单真源，也不能由本 Item 3 记录替代。
...
- 配送订单状态不等于任务状态；本 Item 3 证据不证明任务能力。后续 ADR-49 的任务状态只表达执行
  保管关系，配送订单仍是路线进度真源。
