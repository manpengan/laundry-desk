...
test("createStepUpProof binds pending fields and rejects self-approve", () => {
  ...
  assert.equal(proof.sessionId, OLD_SESSION.sessionId);
  assert.equal(proof.sessionVersion, OLD_SESSION.sessionVersion);
...
test("verifyStepUpProof accepts matching active proof", () => {
  ...
  const result = verifyStepUpProof(proof, pending, pending.createdAt + 10, OLD_SESSION);
...
test("verifyStepUpProof rejects expired proof and self-approve binding", () => {
  ...
  const expired = verifyStepUpProof(proof, pending, pending.createdAt + 1 + 30, OLD_SESSION);
  ...
  const self = verifyStepUpProof(bad, pending, pending.createdAt + 2, OLD_SESSION);
...
test("verifyStepUpProof rejects args_hash mismatch (WYSIWYS)", () => {
  ...
  const result = verifyStepUpProof(tampered, pending, pending.createdAt + 2, OLD_SESSION);
  assert.equal(result.ok, false);
  if (!result.ok) assert.equal(result.reason, "ARGS_HASH_MISMATCH");
});

test("verifyStepUpProof rejects proof issued for an older session of the same creator", () => {
  const store = new MemoryPendingActionStore();
  const pending = seedPending(store);
  const proof = createStepUpProof({
    proofId: "ffffffff-ffff-4fff-8fff-ffffffffffff",
  ...
  const result = verifyStepUpProof(proof, pending, pending.createdAt + 2, NEW_SESSION);

  assert.equal(result.ok, false);
  if (!result.ok) assert.equal(result.reason, "SESSION_BINDING_MISMATCH");

  const advancedVersion = verifyStepUpProof(proof, pending, pending.createdAt + 2, {
    sessionId: OLD_SESSION.sessionId,
    sessionVersion: OLD_SESSION.sessionVersion + 1,
  });
  assert.equal(advancedVersion.ok, false);
  if (!advancedVersion.ok) {
    assert.equal(advancedVersion.reason, "SESSION_BINDING_MISMATCH");
  }
});
