...
export type DeliveryPolicyOperationKind = "command" | "query";
export type DeliveryPolicyRateLimitDecision =
  Readonly<{ allowed: true }> | Readonly<{ allowed: false; retryAfterSeconds: number }>;

export type DeliveryPolicyRateLimiter = Readonly<{
  check(
    kind: DeliveryPolicyOperationKind,
    sessionId: string,
    orgId: string,
    storeId: string,
  ): DeliveryPolicyRateLimitDecision;
}>;

export type DeliveryPolicyRateLimiterOptions = Readonly<{
  maxCommands?: number;
  maxQueries?: number;
  windowMs?: number;
  maxBuckets?: number;
...
function dimension(
  ...
  return createHash("sha256")
    .update("laundry.delivery-policy-rate.v1\0", "utf8")
    .update(kind, "utf8")
    .update("\0", "utf8")
...
export function createDeliveryPolicyRateLimiter(
  options: DeliveryPolicyRateLimiterOptions = {},
): DeliveryPolicyRateLimiter {
  ...
  if (
    ...
    throw new TypeError("Invalid delivery policy rate-limit configuration");
  }
  ...
  return Object.freeze({
    check: (kind, sessionId, orgId, storeId) => {
      ...
      if (!Number.isSafeInteger(now) || now < 0) {
        throw new TypeError("Invalid delivery policy rate-limit clock");
      }
