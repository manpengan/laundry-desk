...
test("CUPS worker submits one new artifact and persists an idempotent receipt", async () => {
...
test("CUPS worker leaves an uncertain marker instead of risking a duplicate print", async () => {
  const dirs = await fixture();
  let submissions = 0;
  try {
    const dependencies = {
...
test("CUPS worker rejects an unavailable configured queue without writing state", async () => {
  const dirs = await fixture();
  try {
...
test("CUPS state drops submitted receipts only after their retained artifact is gone", async () => {
  const dirs = await fixture();
