...
| `src/print/usb-port.ts`        | mock / 受校验设备端口；路径白名单；写超时不挂死                    |
| `src/print/printer-smoke.ts`   | CLI-only path 校验 / USB 自检 ESC/POS / 超时                       |

IPC 只返回 status 视图（id/kind/status/timestamps/error），不暴露设备路径、原始字节、ticketNonce。
renderer 不暴露 printer-smoke IPC；设备诊断仅从操作员 CLI 显式触发。
...
# Linux USB 打印机节点
LAUNDRY_PRINTER_PATH=/dev/usb/lp0 pnpm --filter @laundry/edge-agent printer-smoke -- --validate
LAUNDRY_PRINTER_PATH=/dev/usb/lp0 pnpm --filter @laundry/edge-agent printer-smoke
```

Windows（PowerShell）— COM/LPT/USB 设备端点均接受（裸名称会规范为 `\\.\…`）：
...
- ✅ D5 A/B 状态机骨架
- ✅ M2 受校验设备端口 + CLI-only printer-smoke
