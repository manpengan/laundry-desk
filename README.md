...
当前只推进 Linux 本地 Web Server + Web，覆盖登录/PIN、收件、取衣、客户、会员储值与账户生命周期、付款/欠款、照片、统计、交班、离线恢复、打印、权限、审计，以及局域网只读 Owner Dashboard；macOS App 暂停开发与验收。
...
| 当前阶段   | Linux 本地 Web 正交付 P3 局域网只读 Owner Dashboard；下一批进入 P4 工厂协同最小闭环                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
当前已完成：`Local Foundation → 完整柜台工作日 → 履约/顾客/员工治理 → 本地备份恢复 → 加密离线队列与 Primary → 重放对账 → 会员储值二期 Web 柜台入口（含赠送与退款） → 催取工作台与人工名单 → 账目双口径（日/月/职员） → P2 会员账户生命周期 → P3 局域网 Owner Dashboard → 普通 offline grant → 签名打印软件链 → 独立 Runtime.app 软件`。充值确认会显示服务端冻结的精确赠款与命中档位；店主可从同店局域网的 HTTPS `/owner` 入口只读查看今日四指标及 7/30 日双口径趋势。催取名单仍只供人工联系，不含短信、微信或自动发送；Owner Dashboard 也不包含 AI 分析、云端多店汇总或写操作。macOS 后续开发与验收暂停，云部署和 Windows 仍后置。
...
`P4 工厂协同最小闭环 → later 取送/营销 → macOS/云/Windows 后置`
