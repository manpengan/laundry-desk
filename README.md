...
| 当前阶段   | **阶段 3.1「价目治理」已关闭；下一顺序切片为阶段 3.2 Owner 公网经营与门店管理**。阶段 3.1 已完成 [ADR-39](docs/adr/2026-08-11-adr-39-catalog-governance.md)、本地/真实 PostgreSQL/Browser、PR 与精确主线 CI，并将 `f276bdbf328ae20aba20c7985c690a63484afdca` 发布到 hk-vps 的 `0048_catalog_governance.sql`；API 15/15、Cloud Chromium、marker/schema、catalog golden、权限和清理证据均通过。见[验收记录](docs/superpowers/specs/2026-08-11-stage3-catalog-governance-acceptance.md)与[发布结果](docs/operations/2026-08-11-stage3-catalog-governance-release-result.md)。 |
| 设计真源   | [本地优先产品设计](docs/superpowers/specs/2026-07-25-local-first-v2-product-design.md) · [ADR-16](docs/adr/2026-07-31-adr-16-edge-operations-scope-ratification.md) · [ADR-36](docs/adr/2026-08-09-adr-36-cloud-test-environment.md) · [ADR-37](docs/adr/2026-08-10-adr-37-cloud-web-primary-delivery.md) · [ADR-38](docs/adr/2026-08-11-adr-38-cloud-counter-trust-closure.md) · [ADR-39](docs/adr/2026-08-11-adr-39-catalog-governance.md) · [Cloud Web-first 1–4 交付计划](docs/superpowers/plans/2026-08-10-post-adr36-delivery-plan.md)                               |
| 当前 owner | **Codex** — 设计、实现、集成与门禁                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 目标平台   | 当前：`desk.manpengan.xyz` 上的 Linux Fastify/PostgreSQL + 浏览器 Web；后续桌面、操作系统安装包和实体硬件另行恢复                                                                                                                                                                                                                                                                                                                                                                                                                                                          |

已交付到 hk-vps 的代码面包括 `Local Foundation → 完整柜台工作日 → 履约/顾客/员工治理 → 本地备份恢复 → 加密离线队列与 Primary → 重放对账 → 会员储值二期 → 催取人工名单 → 双口径账目 → 会员账户生命周期 → LAN Owner → Runtime 数据保护/升级 → 正式候选软件证据 → 服务端权威计价/支付退款/件级挂单恢复 → 价目恢复/原子排序/乐观版本/安全审计`。阶段 3.1 的 hk-vps 新鲜证据包含 API 15/15、Cloud Chromium PASS、marker/0048、678-entry golden catalog、应用角色无价目物理删除权限及发布清理；该环境仍禁止真实顾客 PII，也不等于生产 SaaS。当前 macOS 新鲜证据只属于 **software-only**，不冒充 XP-58 实体打印、Developer ID/公证、正式双架构 OCI、Windows 实机或生产云证据。
...
阶段 1、2 与阶段 3.1 已依次关闭；当前下一顺序切片为阶段 3.2 Owner 公网经营与门店管理。
...
[阶段 3.1 结果](docs/operations/2026-08-11-stage3-catalog-governance-release-result.md)。
