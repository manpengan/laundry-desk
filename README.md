...
当前已完成：`Local Foundation → 完整柜台工作日 → 履约/顾客/员工治理 → 本地备份恢复 → 加密离线队列与 Primary → 重放对账 → 会员储值二期（含赠送与退款柜台入口） → 普通 offline grant → 签名打印软件链 → 独立 Runtime.app 软件`。尚未交付 XP-58 实体走样、Developer ID 签名/公证、正式 manifest 签名权威、已签名多架构 OCI，以及 Runtime.app upgrade/rollback；云部署和 Windows 仍后置。
