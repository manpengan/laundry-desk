...
export const shiftClosings = pgTable(
  "shift_closings",
  {
    ...
    retainedFloatCents: integer("retained_float_cents").notNull().default(0),
    expectedCashCents: integer("expected_cash_cents").notNull().default(0),
    cashDifferenceCents: integer("cash_difference_cents").notNull().default(0),
    signatureName: text("signature_name").notNull(),
    periodStartedAt: timestamp("period_started_at", { withTimezone: true, mode: "date" }).notNull(),
    periodEndedAt: timestamp("period_ended_at", { withTimezone: true, mode: "date" }).notNull(),
