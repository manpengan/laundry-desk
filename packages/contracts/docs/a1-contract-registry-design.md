...
- `description` / `description_llm` / `examples`：分别面向人、模型和模型示例，均属于命令/查询定义；示例是 `{ args, description? }` 的 JSON-only 数据，C4 只筛选与格式化，不得补第二套语义或放入真实凭据。secret 命令例外，禁止 examples。
- `invariants` / `sideEffects`：稳定的小写点分**可执行绑定名**，C1 分别解析到不变量检查器与领域事件/副作用注册项；不是自由文本，任何未解析绑定都必须在事务前 fail-closed。
- `data_classification`：`secret` 描述敏感入参，不等于 R5 风险；查询不允许返回 `secret`，且 secret/R5 均不可 AI 投影。
- `input_redaction` / `result_redaction`：分别描述入参日志/投影和返回结果的脱敏路径，不混为一类。

两类规则共用严格结构 `{ path, strategy }`，`strategy` 仅允许
`remove | mask | last4`。`secret` 命令的输入规则只能用 `remove`；C3 必须以
`getInputAuditDisposition(data_classification)` 决定是否记录参数。其对 `secret` 固定返回
`"omit"`，故审计记录不得持久化任何入参 payload，局部规则绝不能被误读成“其余字段可记”。
secret 命令也禁止 examples（包括脱敏/占位示例），避免凭据样例成为另一条泄露通道。
...
6. `data_classification === "secret"` 时必须 `offline_mode === "denied"`，且 `input_redaction` 非空并全部使用 `remove`；C3 根据 `getInputAuditDisposition()` 对 secret 整体省略参数审计，且 secret 不得声明 examples；secret 与 R5 是独立轴，二者都不可 AI 投影。
...
- `max_result_rows` 为必填正安全整数，表达结果条数硬上限；PII 查询不得超过 1000。组织覆盖必须使用 `validateStricterQueryResultLimitOverride()` 且在服务端 SQL/result LIMIT 处应用，不得先取回再在 UI 截断；
...
A1 对命令/查询输入脱敏与规模路径绑定 input schema，并校验 array/number 目标类型；任何
transform/pipe 下无法静态判定的声明路径在定义期拒绝。C1/C3/C4/C5 实现
...
- `input` 必须是真实的严格 `ZodObject`；完整 schema 图中所有嵌套 object 也必须 strict，任意层级均拒绝 `z.any()`、`z.unknown()`、`z.custom()`/`z.instanceof()`、所有 `z.lazy()`、隐式 fallback 的 `default`/`prefault`/`catch`、非法 Date 阈值、非 JSON literal，以及带 `g`/`y` 标志的状态型 RegExp；同时拒绝数组根、strip/passthrough object 和伪造 `parse` 对象。
...
包导出 `isAiProjectableDefinition()` 类型守卫，判据为 `risk !== "R5"` 且 `data_classification !== "secret"`。C4 必须先通过该守卫再投影；契约测试断言 R5 与 secret 都返回 `false`。
