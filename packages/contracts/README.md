...
Laundry Desk V2 的跨层契约包。A1 冻结命令/查询定义，A2 冻结命令信封与错误，A3 冻结租户矩阵、组合键与 RLS SQL 生成契约，A4 冻结 Edge 桥协议；C1 执行总线、C2 生产数据库接线、C4 AI 投影、C5 Policy 求值及实际密码操作均不在本包实现。
...
## A3 租户矩阵、组合键与 RLS SQL

A3 是 **contract-only** 冻结：只导出不可变描述符、fail-closed 校验器和确定性 SQL 文本生成器，
不连接 PostgreSQL、不执行 migration，也不表示 C2 的生产 RLS 已经完成。生产转正仍需正式 schema、
owner/app 角色、事务级 `SET LOCAL`、worker 注入和五类旁路门禁全部实跑。

### 表作用域矩阵

`TENANT_TABLE_MATRIX` 穷举架构 §7 与后续 ADR 实现候选当前声明的表，每张表只出现一次：

| 作用域 | 表                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | RLS 含义                        |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| global | `orgs`、`ai_model_registry`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 平台根/全局注册表，不套租户策略 |
| org    | `stores`、`staffs`、`customers`、`customer_addresses`、`card_types`、`member_cards`、`member_ledger`、`punch_cards`、`points_ledger`、`coupons`、`coupon_grants`、`campaigns`、`referral_rewards`、`notification_templates`、`notification_log`、`settings`、`backups`、`ai_provider_keys`、`ai_presets`、`ai_conversations`、`ai_messages`、`automation_policies`、`approval_requests`、`ai_usage_daily`、`ai_action_log`                                                                                                                                                                                              | 只比较 `org_id`                 |
| store  | `staff_store_roles`、`devices`、`store_features`、`service_types`、`item_catalog`、`addon_catalog`、`color_dict`、`brand_dict`、`remark_dict`、`orders`、`order_lines`、`garments`、`garment_status_log`、`payments`、`production_batches`、`batch_garments`、`production_handoff_attempts`、`production_handoff_attempt_items`、`production_handoff_checkpoints`、`production_handoff_discrepancy_resolutions`、`garment_qc_log`、`delivery_orders`、`print_jobs`、`print_templates`、`shift_closings`、`audit_log`、`edge_devices`、`primary_lease_heads`、`primary_leases`、`ticket_no_blocks`、`ai_pending_actions` | 同时比较 `org_id` 与 `store_id` |

以下是 §7 省略列时的显式冻结选择：

- `stores` 是 org 下的门店目录，`staffs` 可跨店；客户、会员、营销、通知、设置、备份和 AI
  资源保持 org 共享。`automation_policies.store_id?` 只是可选对象过滤，不把策略行降为 store scope。
- ADR-45 以当前 Cloud Web 的单店内部员工交接覆盖早期 contract-only 占位：
  `production_batches`、manifest、attempt/checkpoint、差异处置和 QC 证据全部固定为 store scope；
  跨店、跨组织工厂聚合需要后续 ADR，不能复用旧 org-scope 假设。
- 价目、加项、颜色、品牌和备注字典是门店营业配置，连同设备、单据、打印、交班、Edge/lease
  及带必填 `store_id` 的确认动作使用 store scope。
...
```text
order_lines(org_id, store_id, order_id)
  -> orders(org_id, store_id, id)
garments(org_id, store_id, order_id)
  -> orders(org_id, store_id, id)
...
缺失或空 GUC 会变成 `NULL`：读取匹配不到行，写入被 `WITH CHECK` 拒绝。非空非法 UUID 仍会
转换失败；C8/C2 只能注入服务端认证且已校验的 UUID。
...
| 对象          | 签名方 → 验签方 | 签名 payload 的冻结字段                                                                                      |
| ------------- | --------------- | ------------------------------------------------------------------------------------------------------------ |
| 能力票据      | server → Edge   | `action/job_id/staff_id/device_id/origin/issued_at/exp/nonce`                                                |
| 执行回执      | device → server | `ticket_nonce/result/seq/at`                                                                                 |
| offline grant | server → Edge   | `grant_id/org_id/store_id/staff_id/device_id/permission_version/allowed_commands/issued_at/ttl_ms/not_after` |
| Primary lease | server → Edge   | `lease_id/store_id/device_id/primary_epoch/issued_at/ttl_ms/max_clock_skew_ms/not_after`                     |

### 评审单 §2.1–§2.3：签名范围、canonical 与线路隔离

四类对象统一使用外层包封 `{ protocol_version, payload, sig }`。`sig` 不签自身；每类签名输入是
专属 domain separator、换行与精确 `{ protocol_version, payload }` canonical JSON 的 UTF-8：
`laundry.edge.capability-ticket.v1`、`laundry.edge.execution-receipt.v1`、
`laundry.edge.offline-grant.v1`、`laundry.edge.primary-lease.v1`。公开 API 只提供四个协议专用签发
helper 与 `canonicalizeForSignatureVerification()`，不暴露可绕过 strict schema 的通用签发入口。
canonical serializer 递归按 UTF-16 代码单元字典序写出对象字段（包括整数样式键）、保持数组顺序、
拒绝访问器、稀疏数组、额外数组属性、环、非 JSON 值与非安全整数。协议 payload 先经 strict Zod
schema 解析，所以缺字段、未知字段、非精确 `YYYY-MM-DDTHH:mm:ss.sssZ` 时间与非法数值都会在
canonical 前失败。A5 的 canonical args 具有不同生命周期和语义，不复用 A4 helper；两者只共享
“服务端签发权威前先 canonicalize”的原则。

`not_after` 已按 M0-2 裁定进入 Primary lease 和 offline grant 的 server-signature payload，且
schema 强制 `not_after === issued_at + ttl_ms`。grant 的 M1 `ttl_ms` 硬上限为 12 小时；未来放宽
必须修改契约。`sig` 位于外层，不在 payload 字段清单内。

四类公开类型均明确命名为 `*Signature*Candidate`，表示“待验签候选”，不提供 `Verified*` 品牌或
构造器。候选类型使用不可导出的 unique-symbol 品牌，并以 `WeakMap` 登记具体协议对象；展开、
JSON 往返和手写同形对象都失去登记，能力票据、回执、grant、lease 不能交叉赋值。**登记只证明
线路方向和严格结构来源，不代表密码学验签成功。** D2 必须在执行/入库前，用 server 公钥验
server 线、用登记设备公钥验 device 线，并以同一 canonical bytes 验证签名。

能力票据的 `origin` 只接受不含 path/query/fragment/credentials 的精确 HTTPS Origin 或
`app://host`；具体允许哪些 origin 仍由 Edge 运行时精确 allowlist 决定。

### 评审单 §2.4：grant 只能收紧 A1 上界

组合根必须先导入并执行全部 A1 command 定义，再**无参**调用
`createOfflineGrantRegistrySnapshot()`。该调用由 A1 模块内部枚举当前所有已登记命令并原子封存
注册表；此后任何 `defineCommand()` 都失败。工厂不接受 definitions/manifest 参数，故调用方不能
拿 permissive 子集给自己生成匹配 manifest。`parseServerSignatureOfflineGrantCandidate()` 和
`validateOfflineGrantAllowedCommands()` 只接受持有私有 `WeakMap` 来源的快照，数组、展开对象、
JSON 往返和调用方临时挑选的定义子集都会 fail-closed。快照公开的 `definition_refs` 仅供观测与
审计，不是完整性输入。每个白名单名字必须对应有来源登记的 command，且
`offline_mode !== "denied"`；未知、query 或任一受支持版本为 denied 的条目一律 fail-closed；
多版本并存时取最严格档位。校验结果会单列
`primary_lease_commands`：这些命令即使在 grant 白名单内，运行时仍必须额外证明有效 Primary
lease，grant 不能把它降级成普通离线授权。

可证伪性已实跑：人为移除 `offline_mode === "denied"` 分支后，定向负例
`rejects a grant that names an offline-denied command` 从绿变红并报“expected function to throw”；
恢复分支后同一测试重新通过。

### 评审单 §2.5：能力票据与 grant 也采用可信单调时间

能力票据额外签入 `issued_at`，并强制 `exp > issued_at`。D2/Edge 必须以
`authorized_duration_ms = exp - issued_at`，grant 则直接使用签名 `ttl_ms`。两者都在发请求前取得
monotonic anchor，再按
`local_deadline = request_start_mono + authorized_duration_ms - safety_margin_ms` 建立本地截止。
RTT 大于等于授权时长、进程/OS 重启、休眠恢复、单调钟回退或无法证明时间连续性时一律
fail-closed；不得以 `Date.now()` 直接判断票据、grant 或 lease 权力仍有效。A4 仅冻结字段与约束，
不在契约包伪造一套未实测的时钟状态机；D2 必须复用 M0-2 的连续性实现与静态禁墙钟门禁。

### 评审单 §2.6–§2.9：队列归属、版本、水位与措辞边界

`EdgeQueueEnvelope` 独占 A2 挂起的 replay 字段。普通 grant 分支只携带 `grant_id`；
`primary_lease` 分支同时携带 `grant_id + lease_id + primary_epoch + per_lease_seq`，其中 epoch/seq
均为正安全整数。队列信封不接受 `actor`、`tenant`、`org_id` 或 `store_id` 自报字段；重连后 C8
仍从认证设备会话及服务端 grant/lease 记录注入身份与租户。

`queue_envelope_version` 与 `contracts_major` 是两套版本：前者描述本地持久化/回放信封，后者是
ADR-08 的协议兼容单位（在 1.0 前允许为 `0`）。
`classifyQueueEnvelopeCompatibility(envelope, policy)` 会先解析**实际信封**，再原子检查两套版本；
只有 queue 版本位于 `minimum_secure_queue_version..current_queue_version` 且 contracts major 是
current/previous 时才返回 `{ mode: "replay", automatic_replay: true }`。过旧 queue 或 contracts
major 转 `recover_to_arbitration`；回滚后遇到任一未来版本转 `read_only_recovery`。二者都以类型
固定 `automatic_replay: false`，调用方不能先分类一个版本再重放另一个 payload。

每 lease 高水位、精确重复 hash、同 seq 内容碰撞、released/stale epoch 与乱序判定都是 server
持久化的运行时状态；单条契约只能保证 tuple 形状、正 seq 与 A2 幂等键。旧 epoch 回执仍进入
不可变审计，但不得自动应用领域写入。三元组职责仅是幂等、防重放、顺序与审计归属；它**不防
