...
- `production_batches` 可承载同 org 的店厂聚合，保持 org scope；具体 `batch_garments` 行引用某店
  garment，使用 store scope。
