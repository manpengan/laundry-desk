...
const required = (name) => {
  ...
  if (value === undefined || value.length === 0) {
    throw new Error("DELIVERY_ORDERS_PG_INPUT_REQUIRED");
  }
  return value;
};
...
if (
  ...
  throw new Error("DELIVERY_ORDERS_PG_ISOLATION_REQUIRED");
}
...
const exitCode = await new Promise((resolve, reject) => {
  const child = spawn(
    process.execPath,
    ["--test", "--test-concurrency=1", "apps/server/dist/delivery-orders/pg-invariants.test.js"],
...
if (exitCode !== 0) throw new Error("DELIVERY_ORDERS_PG_ACCEPTANCE_FAILED");
process.stdout.write("ADR48_DELIVERY_ORDERS_PG_ACCEPTANCE_OK\n");
