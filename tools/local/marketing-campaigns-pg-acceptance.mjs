...
if (
  ...
  throw new Error("MARKETING_CAMPAIGNS_PG_ISOLATION_REQUIRED");
}
...
const exitCode = await new Promise((resolve, reject) => {
  const child = spawn(
    process.execPath,
    ["--test", "--test-concurrency=1", "apps/server/dist/marketing/pg-invariants.test.js"],
    { cwd: process.cwd(), env: environment, shell: false, stdio: "inherit" },
  );
  child.once("error", reject);
  child.once("exit", (code) => resolve(code ?? 1));
});
if (exitCode !== 0) throw new Error("MARKETING_CAMPAIGNS_PG_ACCEPTANCE_FAILED");
process.stdout.write("ADR52_MARKETING_CAMPAIGNS_PG_ACCEPTANCE_OK\n");
