...
const required = (name) => {
  ...
  if (value === undefined || value.length === 0) {
    throw new Error("DELIVERY_EVIDENCE_PG_INPUT_REQUIRED");
  }
  return value;
};
...
if (
  ...
  throw new Error("DELIVERY_EVIDENCE_PG_ISOLATION_REQUIRED");
}

const config = await loadLocalConfig({ env: process.env });
const databaseUrl = (username, password) => {
  const url = new URL(
    `postgresql://127.0.0.1:${process.env.LAUNDRY_PG_HOST_PORT ?? "8543"}/laundry_v2`,
  );
...
const exitCode = await new Promise((resolve, reject) => {
  const child = spawn(
    process.execPath,
    ["--test", "--test-concurrency=1", "apps/server/dist/delivery-evidence/pg-invariants.test.js"],
...
if (exitCode !== 0) throw new Error("DELIVERY_EVIDENCE_PG_ACCEPTANCE_FAILED");
process.stdout.write("ADR51_DELIVERY_EVIDENCE_PG_ACCEPTANCE_OK\n");
