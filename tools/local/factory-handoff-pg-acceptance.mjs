...
const required = (name) => {
  ...
  if (value === undefined || value.length === 0) {
    throw new Error("FACTORY_HANDOFF_PG_INPUT_REQUIRED");
  }
  return value;
};
...
if (
  ...
  throw new Error("FACTORY_HANDOFF_PG_ISOLATION_REQUIRED");
}
...
const exitCode = await new Promise((resolve, reject) => {
  const child = spawn(
    ...
    [
      "--test",
      "--test-concurrency=1",
      "apps/server/dist/fulfillment/pg-factory-invariants.test.js",
      "apps/server/dist/fulfillment/pg-fulfillment-lock-order.test.js",
    ],
...
if (exitCode !== 0) throw new Error("FACTORY_HANDOFF_PG_ACCEPTANCE_FAILED");
process.stdout.write("ADR45_FACTORY_HANDOFF_PG_ACCEPTANCE_OK\n");
